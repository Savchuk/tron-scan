...
class TronApi{
  ...
  function getBalance($address,$confirmed=true){
    ...
    if(!isset($accountInfo->balance)){
      return 0; // contract maybe has zero balance - 2020-11-2
  ...
  function createAssetIssue($opts){  
    $opts['name'] =  bin2hex($opts['name']);
    $opts['abbr'] = bin2hex($opts['abbr']);
    $opts['description'] = bin2hex($opts['description']);
    $opts['url'] = bin2hex($opts['url']);
    $opts['owner_address'] = Address::decode($opts['owner_address']);
    return $this->fullNode->post('/wallet/createassetissue',$opts);
  }
  ...
  //patch 2020-11-2
  function triggerConstantContract($contract,$function,$parameter,$from,$confirmed=true){
    $payload = [
      'contract_address' => Address::decode($contract),
      'function_selector' => $function,
      'parameter' => $parameter,
